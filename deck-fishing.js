(() => {
  ...
  function saveAll() { localStorage.setItem(STORAGE.count, String(count)); localStorage.setItem(STORAGE.dex, JSON.stringify(dex)); localStorage.setItem(STORAGE.bag, JSON.stringify(bag.slice(-80))); localStorage.setItem(STORAGE.money, String(pearls)); localStorage.setItem(STORAGE.letters, JSON.stringify(savedLetters.slice(-50))); if (best) localStorage.setItem(STORAGE.best, JSON.stringify(best)); }
  ...
  function pickLetter(){ const wilburd = LETTERS.filter(l=>l.title.includes('威爾伯德')); const normal = LETTERS.filter(l=>!l.title.includes('威爾伯德')); return (Math.random() < 0.72 ? wilburd : normal)[Math.floor(Math.random() * (Math.random() < 0.72 ? wilburd.length : normal.length))]; }
  ...
  function showCard(item){
    ...
    if(item.kind==='letter'){
      card.innerHTML=`<div style="font-size:22px;color:${color}">${icon} ${item.letterTitle || item.name}</div><div>海域：${item.zone}<br>稀有度：${item.rarity}<br>FishDex：${got}/${CATCHES.length}<br>已讀信件：${savedLetters.length}</div><div class="letter-text">${item.letter}</div>`;
    } else {
  ...
  function openDex(){
    ...
    panel.innerHTML=`<button class="fishdex-close" type="button">關閉</button><h2>📖 FishDex</h2><div>收集：${got}/${CATCHES.length}｜總釣獲：${count}｜珍珠：${pearls}｜最大：${bestText}</div><div class="fishdex-actions"><button id="sellFishBtn" type="button">賣出背包可販售漁獲</button><button id="clearFishCardBtn" type="button">只關閉</button></div><h3>最近漁獲</h3><div class="fish-grid">${recent.length?recent.map(it=>`<div class="fish-entry"><strong>${iconFor(it)} ${it.quality} ${it.name}</strong><small>${it.rarity}｜${it.weight.toFixed(2)}kg｜${it.kind==='trash'?'不可販售':priceOf(it)+' 珍珠'}</small></div>`).join(''):'<div class="fish-entry">還沒有背包漁獲，先去釣魚吧。</div>'}</div><h3>最近瓶中信</h3><div class="fish-grid">${letters.length?letters.map(l=>`<div class="fish-entry"><strong>🍾 ${l.title}</strong><small>${l.text}</small></div>`).join(''):'<div class="fish-entry">還沒有讀過瓶中信。</div>'}</div>`;
