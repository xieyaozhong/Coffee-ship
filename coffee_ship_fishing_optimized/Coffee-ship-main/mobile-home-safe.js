(() => {
  ...
  function mountRoleCollapse() {
    ...
    wrapper.id = 'roleCollapseBox';
    wrapper.className = 'role-collapse-box';
    wrapper.innerHTML = '<summary>👑 特殊角色登入</summary><div class="role-collapse-content"></div>';
    roleBox.insertAdjacentElement('beforebegin', wrapper);
    wrapper.querySelector('.role-collapse-content').appendChild(roleBox);
  }
  ...
  function addStyle() {
    ...
    style.textContent = `
      ...
      @media(max-width:760px){.creator>.hint{display:none!important}.quick-help-content{font-size:13px}.role-collapse-box:not([open]){margin-bottom:0}}
    `;
  ...
  function init() {
    ...
    const timer = setInterval(() => {
      mountRoleCollapse();
      renameRandom();
      if (document.getElementById('roleCollapseBox') && document.getElementById('randomAnimalBtn')) clearInterval(timer);
    }, 250);
