(() => {
  'use strict';
  if (window.__COFFEE_SHIP_AUCTION_SHIP_V1__) return;
  window.__COFFEE_SHIP_AUCTION_SHIP_V1__ = true;

  const PANEL_ID = 'abyssAuctionEvent';
  const BAG_KEY = 'coffeeShipFishBag';
  const STATE_KEY = 'coffeeShipAuctionShipState';
  const LEGACY_STATE_KEY = 'coffeeShipAbyssAuctionState';
  const MAX_BAG = 240;
  const TURN_MS = 8000;
  const BASE_CHANCE = .055;
  const MIN_CASTS_BETWEEN_VISITS = 6;
  const MIN_TIME_BETWEEN_VISITS = 2 * 60 * 1000;
  ...
  function spend(amount,reason) {
    ...
    if (economy()?.spend) return economy().spend(value,reason,{source:'auction-ship'});
  ...
  function auctionState() {
    const defaults = {visits:0,wins:0,losses:0,spent:0,collected:[],castsSince:99,lastSeenAt:0,lastCastId:''};
    const current = read(STATE_KEY,null);
    const legacy = read(LEGACY_STATE_KEY,defaults);
    const state = current && typeof current === 'object' ? {...defaults,...current} : {...defaults,...legacy};
    for (const key of ['visits','wins','losses','spent','castsSince','lastSeenAt']) state[key] = Math.max(0,Number(state[key] || 0));
    state.collected = Array.isArray(state.collected) ? state.collected : [];
    state.lastCastId = String(state.lastCastId || '');
    return state;
  }
  ...
  function addTreasure(treasure,price,castId) {
    ...
    window.dispatchEvent(new CustomEvent('coffee-ship:bag-changed',{detail:{source:'auction-ship',item}}));
    return item;
  }
  ...
  function ensurePanel() {
    ...
    panel.setAttribute('aria-label','拍賣船競標事件');
  ...
  function render() {
    ...
    ensurePanel().innerHTML = `<section class="aa-shell" style="--aa-accent:${treasure.accent}"><header class="aa-head"><div class="aa-title"><span class="aa-hammer">🚢</span><div><h3>黑金拍賣船</h3><p>獨立船隻事件・每次航行只競標一件珍寶</p></div></div><button class="aa-close" type="button" ${active.phase==='npc'?'disabled':''}>${bidding?'放棄':'關閉'}</button></header><div class="aa-status"><span>${bidding?`出價回合 ${active.round}・第 ${TREASURES.indexOf(treasure)+1}/10 種珍寶`:'競標已結束'}</span><span class="aa-wallet">🦪 <b id="abyssAuctionBalance">${format(balance())}</b> 珍珠</span></div>${active.phase==='result'?renderResult():renderBidding()}<footer class="aa-footer"><span>拍賣船不屬於風險事件，只有得標時才扣珍珠。</span><span>${active.npcs.filter(npc => !npc.out).length} 名 NPC 尚未退出</span></footer></section>`;
  }
  ...
  function finishWin() {
    ...
    stopTimers();
    const treasure = active.treasure;
    const payment = spend(active.currentBid,`拍賣船：${treasure.name}`);
    if (!payment.ok) {
    ...
    const text = `黑金拍賣槌落下，你以 🦪 ${format(active.currentBid)} 珍珠擊退所有競標者。\n${item.name}已放入背包。`;
  ...
  function npcResponse() {
    ...
    if (!candidates.length) {
      active.logs.push('船艙內全場沉默，拍賣官準備落槌。');
      render();
  ...
  function closeAuction() {
    ...
    document.body.classList.remove('auction-ship-open','abyss-auction-open');
    active = null;
    return true;
  }
  ...
  function competingBusy() {
    return [
      ...
      'ocean-friends-qte-open','salvage-qte-open','auction-ship-open','abyss-auction-open'
  ...
  function openAuction(castId,forced=false) {
    ...
    active = {
      castId:castId || `auction_ship_${Date.now()}`,
      treasure,
      ...
      round:1,
      turnMs:TURN_MS,
      logs:[`🚢 黑金拍賣船宣布${treasure.name}起標價為 🦪 ${format(treasure.startBid)}。`],
      result:null
    };
    const state = auctionState();
    state.visits += 1;
    ...
    document.body.classList.add('auction-ship-open','abyss-auction-open');
    pushEvent('黑金船靠舷',`一艘掛著金色槌旗的拍賣船靠近甲板。\n本場拍品：${treasure.icon} ${treasure.name}［${treasure.rarity}］\n起標價：🦪 ${format(treasure.startBid)} 珍珠。`,treasure.icon,treasure.accent);
    startTurnTimer();
    if (forced) {
      active.logs.push('測試模式：拍賣船由系統手動召來。');
      render();
    }
    return true;
  }
  ...
  function scheduleOpen(castId,attempt=0) {
    ...
    if (attempt < MAX_QUEUE_ATTEMPTS && window.COFFEE_SHIP_DECK?.isDeckOpen?.()) {
      setTimeout(() => scheduleOpen(castId,attempt+1),700);
    } else {
      pendingCastId = '';
    }
  }
  ...
  function processFishingResult(event) {
    ...
    const state = auctionState();
    if (state.lastCastId === detail.castId) return;
    state.castsSince += 1;
    saveAuctionState(state);
    if (active || pendingCastId) return;
    ...
    pendingCastId = detail.castId;
    pushEvent('遠方旗號',`霧中出現金色拍賣槌旗號，黑金拍賣船正在靠近。`, '🚢','#f2a957');
    scheduleOpen(detail.castId);
  }

  function detachRiskEvent() {
    const api = window.COFFEE_SHIP_FISHING_ADVENTURES;
    if (!Array.isArray(api?.events)) return false;
    let removed = false;
    for (let index=api.events.length-1;index>=0;index-=1) {
      if (api.events[index]?.id !== 'abyss_auction') continue;
      api.events.splice(index,1);
      removed = true;
    }
    if (removed) {
      window.dispatchEvent(new CustomEvent('coffee-ship:auction-detached-from-risk',{detail:{eventId:'abyss_auction',standalone:true}}));
    }
    return true;
  }
  ...
  function bind() {
    ...
    window.addEventListener('coffee-ship:fishing-adventures-ready',detachRiskEvent);
    window.addEventListener('coffee-ship:fishing-adventure-expanded',detachRiskEvent);
    ...
    window.addEventListener('coffee-ship:scene',event => {
      if (event.detail?.scene !== 'deck' && active?.phase !== 'result') finishLoss('你離開甲板，拍賣船取消了你的競標資格。');
      else if (event.detail?.scene !== 'deck' && active) closeAuction();
      if (event.detail?.scene !== 'deck') pendingCastId = '';
    });
  }
  ...
  function init() {
    ...
    const detachTimer = setInterval(() => {
      attempts += 1;
      if (detachRiskEvent() || attempts > 80) clearInterval(detachTimer);
    },250);

    const api = {
      open:() => openAuction(window.COFFEE_SHIP_FISHING_API?.getCurrentCastId?.() || `auction_ship_test_${Date.now()}`,true),
    ...
    window.COFFEE_SHIP_AUCTION_SHIP = api;
    window.COFFEE_SHIP_ABYSS_AUCTION = api;
    window.dispatchEvent(new CustomEvent('coffee-ship:auction-ship-ready',{detail:{treasures:TREASURES.length,standalone:true,version:1}}));
    window.dispatchEvent(new CustomEvent('coffee-ship:abyss-auction-ready',{detail:{treasures:TREASURES.length,standalone:true,version:1}}));
  }
