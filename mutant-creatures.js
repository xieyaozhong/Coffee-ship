(() => {
  'use strict';
  if (window.__COFFEE_SHIP_MUTANT_CREATURES_V2__) return;
  window.__COFFEE_SHIP_MUTANT_CREATURES_V2__ = true;
  ...
  function trigger() {
    ...
    const item = {
      ...
      kind:'mutant',icon:mutant[0],trait:mutant[3],at:Date.now()
    };
    const bag = read('coffeeShipFishBag', []);
    bag.push(item);
    ...
    window.COFFEE_SHIP_FISHING_API?.pushEvent?.({
      ...
      text:`稀有度：${item.rarity}。重量：${item.rarity === '世界級' ? '∞' : weight.toFixed(2) + ' kg'}。特性：${item.trait}。${loss}`
    });
  }
