(() => {
  ...
  const pool = [
    ...
    {icon:'🐯',name:'狂歡節老虎面具',rarity:'稀有',quality:'動物面具',weight:.35,note:'額頭畫著奇怪的紅色條紋。'},
    ...
    {icon:'👔',name:'狂歡節燕尾服',rarity:'史詩',quality:'漂流衣物',weight:.72,note:'胸口別著一朵枯萎的花。'},
    {icon:'👗',name:'狂歡節晚禮服',rarity:'稀有',quality:'漂流衣物',weight:.28,note:'裙擺縫著亮片。'},
    {icon:'🧥',name:'狂歡節披風',rarity:'傳說',quality:'漂流衣物',weight:.44,note:'披風內側寫滿邀請名單。'},
    {icon:'🧤',name:'狂歡節手套',rarity:'普通',quality:'漂流衣物',weight:.04,note:'指尖沾著金粉。'},
    {icon:'🧣',name:'狂歡節圍巾',rarity:'普通',quality:'漂流衣物',weight:.1,note:'聞起來像糖漿和海風。'},
    {icon:'👒',name:'狂歡節羽毛帽',rarity:'稀有',quality:'漂流衣物',weight:.14,note:'羽毛顏色過於鮮豔。'},
    {icon:'🎩',name:'狂歡節高帽',rarity:'史詩',quality:'漂流衣物',weight:.32,note:'帽底藏著一張小丑票券。'},
    {icon:'👠',name:'狂歡節高跟鞋',rarity:'史詩',quality:'漂流衣物',weight:.5,note:'鞋跟裡藏著小鈴鐺。'},
    {icon:'👢',name:'狂歡節長靴',rarity:'稀有',quality:'漂流衣物',weight:.78,note:'靴底踩著乾掉的彩紙。'},
    {icon:'👞',name:'狂歡節皮鞋',rarity:'普通',quality:'漂流衣物',weight:.46,note:'只剩左腳。'},
    {icon:'🎀',name:'狂歡節絲帶',rarity:'普通',quality:'漂流衣物',weight:.02,note:'打了一個解不開的結。'},
    // 玩具 15
    ...
    {icon:'🎲',name:'狂歡節骰子',rarity:'普通',quality:'玩具',weight:.05,note:'每一面都是六點。'},
    {icon:'🧸',name:'狂歡節玩偶熊',rarity:'稀有',quality:'玩具',weight:.36,note:'肚子裡有沙沙聲。'},
    {icon:'🪅',name:'狂歡節彩罐',rarity:'稀有',quality:'玩具',weight:.42,note:'裡面敲不出糖果，只掉出彩灰。'},
    {icon:'🪇',name:'狂歡節沙鈴',rarity:'普通',quality:'玩具',weight:.18,note:'搖起來像遠方的腳步聲。'},
    {icon:'🎪',name:'狂歡節雜耍球',rarity:'普通',quality:'玩具',weight:.12,note:'三顆球永遠找不到第四顆。'},
    // 飾品 10
    ...
    {icon:'📜',name:'狂歡節貴族徽章',rarity:'稀有',quality:'飾品',weight:.04,note:'背面寫著貴賓席。'},
    {icon:'🏅',name:'狂歡節勳章',rarity:'史詩',quality:'飾品',weight:.08,note:'獎給最會笑的人。'},
    {icon:'💠',name:'狂歡節水晶胸針',rarity:'傳說',quality:'飾品',weight:.06,note:'水晶中有舞會倒影。'},
    {icon:'✨',name:'狂歡節金色耳環',rarity:'稀有',quality:'飾品',weight:.02,note:'晃動時會灑出金粉。'},
    {icon:'🔔',name:'狂歡節銀鈴腳環',rarity:'史詩',quality:'飾品',weight:.07,note:'鈴聲只在夜裡響起。'}
  ];
  ...
  function normalizeOldCarnivalLoot() {
    ...
    const next = bag.map(item => {
      ...
      if (/狂歡|華麗羽毛帽/.test(item.name || '') && item.zone !== '狂歡島遺失物') {
        changed = true;
        return { ...item, zone:'狂歡島遺失物' };
      }
      if (item.name === '狂歡面具' || item.name === '狂歡節面具') {
        changed = true;
        return pick();
      }
      return item;
