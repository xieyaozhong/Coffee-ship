...
- `coffee-ship-fish-pixel-icons.js`：為每個魚種即時生成穩定且獨特的 24×24 像素圖示，並提供八種鯊魚與二十種變異生物的專屬輪廓與花紋。
- `coffee-ship-item-pixel-icons.js`：為物品生成獨特像素圖示，並讓同系列漂流瓶共用固定瓶身。
- `coffee-ship-mermaid-pixel-icons.js`：依粉紫髮、紫藍魚尾與青色水光的統一風格，生成十二種美人魚事件專屬像素圖示。
