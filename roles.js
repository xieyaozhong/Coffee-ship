(() => {
  ...
  const PENDING_ROLE_KEY = 'coffeeShipPendingRoleLogin';
  const roles = {
  ...
  function play(notes) {
    try{
      ...
      const audio=new AC();const now=audio.currentTime+.03;
      notes.forEach((freq,i)=>{const osc=audio.createOscillator(),gain=audio.createGain();osc.type='sine';osc.frequency.setValueAtTime(freq,now+i*.22);gain.gain.setValueAtTime(.0001,now+i*.22);gain.gain.exponentialRampToValueAtTime(.08,now+i*.22+.03);gain.gain.exponentialRampToValueAtTime(.0001,now+i*.22+.2);osc.connect(gain);gain.connect(audio.destination);osc.start(now+i*.22);osc.stop(now+i*.22+.24);});
    }catch(error){console.warn('role audio failed',error);}
  }

  function fx(role) {
    const count=role.heart?28:12;
    for(let i=0;i<count;i++){
      const el=document.createElement('div');el.className=role.heart?'heart-shot':'role-fx';el.textContent=role.heart?(i%4===0?'開心+20':'❤️'):(i%3===0?'♪':role.icon);el.style.left=`${20+Math.random()*60}vw`;el.style.top=`${45+Math.random()*35}vh`;el.style.color=role.color;
      if(role.heart){const angle=Math.PI*2*i/count+Math.random()*.25,dist=90+Math.random()*170;el.style.setProperty('--dx',`${Math.cos(angle)*dist}px`);el.style.setProperty('--dy',`${Math.sin(angle)*dist}px`);}
      document.body.appendChild(el);setTimeout(()=>el.remove(),1700);
    }
    if(role.heart){const toast=document.createElement('div');toast.className='happy-toast';toast.textContent='❤️ 女僕服務讓大家都開心起來了！';document.body.appendChild(toast);setTimeout(()=>toast.remove(),2200);}
  }

  function showPanel(role,name) {
    const gamePanel=document.getElementById('gamePanel');if(!gamePanel)return;
    document.querySelector('.role-panel')?.remove();
    const panel=document.createElement('div');panel.className='role-panel';panel.innerHTML=`<div class="role-title">${role.icon} ${name}｜${role.role}</div><div class="role-power">能力：${role.power}</div><button class="role-power-btn" type="button">使用能力</button>`;
    const btn=panel.querySelector('button');btn.style.background=role.color;btn.addEventListener('click',()=>{play(role.notes);fx(role);});gamePanel.appendChild(panel);
  }

  function persistRole(role,name) {
    ...
    const avatar=parseJson(localStorage.getItem('coffeeShipAvatar'),{});
    localStorage.setItem('coffeeShipAvatar',JSON.stringify({...avatar,name,animal:'human'}));
    window.COFFEE_SHIP_PENDING_ROLE=roleState;
    window.COFFEE_SHIP_FORCE_HUMAN_ROLE=true;
    return roleState;
  }

  function launchRole(role,name) {
    persistRole(role,name);
    const startBtn=document.getElementById('startBtn');
    const playerNameInput=document.getElementById('playerName');
    if(playerNameInput)playerNameInput.value=name;
    if(startBtn)startBtn.click();
    setTimeout(()=>{
      const creator=document.getElementById('creator'),gamePanel=document.getElementById('gamePanel'),avatarName=document.getElementById('avatarName');
      if(creator&&gamePanel&&gamePanel.classList.contains('hidden')){creator.classList.add('hidden');gamePanel.classList.remove('hidden');}
      if(avatarName)avatarName.textContent=`${role.icon} ${name}｜${role.role}`;
      showPanel(role,name);
    },220);
  }

  function safeEnterGame(role,name,code) {
    const previousAvatar=parseJson(localStorage.getItem('coffeeShipAvatar'),{});
    const previousAnimal=previousAvatar.animal||localStorage.getItem('coffeeShipAnimal')||'human';
    persistRole(role,name);
    if(previousAnimal!=='human'){
      localStorage.setItem(PENDING_ROLE_KEY,JSON.stringify({code,name,createdAt:Date.now()}));
      document.documentElement.classList.add('login-resetting');
      setTimeout(()=>window.location.reload(),60);
      return;
    }
    launchRole(role,name);
  }

  function enterRole(code) {
    const normalized=String(code||'').trim().toUpperCase();
    const role=roles[normalized];const note=document.getElementById('roleCodeNote');
    if(!role){if(note)note.textContent='找不到這個角色編號，請從上方角色卡重新選擇。';return;}
    const name=`${pick(role.names)}${suffix()}`;
    if(note)note.textContent=`已啟用：${role.icon} ${role.role}，正在準備精緻人類角色……`;
    safeEnterGame(role,name,normalized);
  }

  function resumePendingRole() {
    const pending=parseJson(localStorage.getItem(PENDING_ROLE_KEY),null);
    if(!pending?.code)return false;
    const role=roles[pending.code];
    localStorage.removeItem(PENDING_ROLE_KEY);
    if(!role)return false;
    const name=pending.name||`${pick(role.names)}${suffix()}`;
    setTimeout(()=>launchRole(role,name),120);
    return true;
  }
  ...
  function mount() {
    const startBtn=document.getElementById('startBtn');
    if(!startBtn)return;
    if(!document.getElementById('roleCode')){
      const box=document.createElement('div');box.className='role-code-box';box.innerHTML=`<div class="role-code-row"><input id="roleCode" maxlength="20" autocomplete="off" autocapitalize="characters" placeholder="輸入角色編號"><button id="roleEnterBtn" type="button">特殊角色登船</button></div><p id="roleCodeNote" class="role-code-note">特殊角色：VIOLIN2026、SINGER2026、PIRATE2026、MAID2026</p>`;
      ...
      document.getElementById('roleCode').addEventListener('keydown',event=>{if(event.key==='Enter'){event.preventDefault();enterRole(event.target.value);}});
    }
    resumePendingRole();
  }
  ...
  }catch(error){console.error('role system failed',error);}
})();
