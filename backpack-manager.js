(() => {
  ...
  function letterSources() {
    const sources = [['coffeeShipBottleLetters','😂🍾'],['coffeeShipLanarLetters','🌊🧪'],['coffeeShipArielLetters','🧜‍♀️💔'],['coffeeShipIslandLetters','🏝️💌'],['coffeeShipBlackbeardLetters','🏴‍☠️🗺️'],['coffeeShipMadPriestLetters','📜🕯️'],['coffeeShipCarnivalLetters','🎭🍾'],['coffeeShipTurtleSoupLetters','🍲🐢']];
    const out = [];
    sources.forEach(([key, icon]) => read(key, []).forEach((x, i) => out.push({ key, index: i, icon: x.icon || icon, title: x.title, text: x.text, series: x.series, rarity: x.rarity, at: x.at || 0 })));
  ...
  function renderLetters() { const arr = letterSources(); if (!arr.length) return '<div class="backpack-empty">目前沒有信件。</div>'; return `<div class="backpack-list">${arr.map(l => `<div class="backpack-entry"><strong>${l.icon} ${l.title}</strong><small>${l.series ? '系列：' + l.series + '<br>' : ''}${l.rarity ? '稀有度：' + l.rarity + '<br>' : ''}${l.text || ''}<br><span class="price-line">售價：${letterPrice(l)} 珍珠</span></small><div class="backpack-actions"><button class="sell-btn" data-sell-letter-key="${l.key}" data-sell-letter-index="${l.index}">販售</button><button class="discard-btn" data-discard-letter-key="${l.key}" data-discard-letter-index="${l.index}">丟棄</button></div></div>`).join('')}</div>`; }

  function panel() { return document.getElementById('fishDexPanel'); }
  function ensurePanel() {
    let p = panel();
    if (p) return p;
    const host = document.getElementById('gamePanel') || document.querySelector('.shell') || document.body;
  ...
  function closeBackpack() { const p = panel(); if (!p) return; p.classList.add('hidden'); p.classList.remove('backpack-safe-panel'); p.style.display = 'none'; lastVisible = false; }
  function forceBuild() {
    const p = ensurePanel();
  ...
  function bind() {
    document.addEventListener('click', e => {
      const closeBtn = e.target.closest?.('[data-close-backpack]'); if (closeBtn) { closeBackpack(); e.preventDefault(); return; }
