(() => {
  ...
  function isFish(item) {
    return item && !isCurrency(item) && (item.kind === 'fish' || item.kind === 'mutant' || (!item.kind && item.weight && !/瓶|信|圖|殘頁|面具|帽|鞋|杯|戒|鏡|玩具|布偶|手套|緞帶/.test(item.name || '')));
  }
  ...
  function itemTitle(item) {
    const icon = item.icon || (item.kind === 'mutant' ? '🧬' : isCurrency(item) ? '🦪' : isItem(item) ? '📦' : '🐟');
    return `${icon} ${item.quality && item.kind !== 'currency' ? item.quality + ' ' : ''}${item.name || '未知物品'}`;
  }
  ...
  function letterPrice(letter) {
    const t = `${letter.title || ''} ${letter.text || ''}`;
    if (/黑鬍子|藏寶圖/.test(t)) return 120;
    if (/拉納爾|愛麗兒/.test(t)) return 95;
    if (/瘋狂神父|狂歡島/.test(t)) return 80;
    if (/哈斯|可可|莫納|孤島/.test(t)) return 70;
    return 45;
  }
  ...
  function sellLetter(key, index) {
    ...
    const price = letterPrice(letter || {});
    list.splice(index, 1);
    save(key, list);
    addPearls(price, `販售：${letter?.title || '信件'}`);
    activeTab = 'item';
    forceBuild();
  }
  ...
  function renderFish(items) {
    ...
    return `<div class="backpack-list">${arr.map(({item,index}) => `<div class="backpack-entry"><strong>${itemTitle(item)}</strong><small>${itemDetail(item)}</small><div class="backpack-actions"><button class="sell-btn" data-sell-bag="${index}">販售</button><button class="discard-btn" data-discard-bag="${index}">丟棄</button></div></div>`).join('')}</div>`;
  }
  function renderItems(items) {
    ...
    return `<div class="backpack-list">${arr.map(({item,index}) => `<div class="backpack-entry"><strong>${itemTitle(item)}</strong><small>${itemDetail(item) || '特殊物品'}</small><div class="backpack-actions"><button class="sell-btn" data-sell-bag="${index}">販售</button><button class="discard-btn" data-discard-bag="${index}">丟棄</button></div></div>`).join('')}</div>`;
  }
  function renderLetters() {
    const arr = letterSources();
    if (!arr.length) return '<div class="backpack-empty">目前沒有信件。</div>';
    return `<div class="backpack-list">${arr.map(l => `<div class="backpack-entry"><strong>${l.icon} ${l.title}</strong><small>${l.text || ''}<br><span class="price-line">售價：${letterPrice(l)} 珍珠</span></small><div class="backpack-actions"><button class="sell-btn" data-sell-letter-key="${l.key}" data-sell-letter-index="${l.index}">販售</button><button class="discard-btn" data-discard-letter-key="${l.key}" data-discard-letter-index="${l.index}">丟棄</button></div></div>`).join('')}</div>`;
  }
  ...
  function bind() {
    document.addEventListener('click', e => {
      ...
      const sellBagBtn = e.target.closest?.('[data-sell-bag]');
      if (sellBagBtn) { sellBagIndex(Number(sellBagBtn.dataset.sellBag)); e.preventDefault(); return; }
      ...
      const sellLetterBtn = e.target.closest?.('[data-sell-letter-key]');
      if (sellLetterBtn) { sellLetter(sellLetterBtn.dataset.sellLetterKey, Number(sellLetterBtn.dataset.sellLetterIndex)); e.preventDefault(); return; }
