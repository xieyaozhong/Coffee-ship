(() => {
  ...
  function allLetters() {
    ...
    return uniqueByTitle(base.concat(lanar, ariel, island, blackbeard).sort((a, b) => b.at - a.at));
  }
  ...
  function enhanceFishDex() {
    ...
    const letterHtml = `<h3>最近瓶中信 / 漂流瓶</h3><div class="fish-grid">${letters.length ? letters.slice(0, 36).map(l => `<div class="fish-entry"><strong>${l.source} ${l.title}</strong><small>${l.text}</small></div>`).join('') : '<div class="fish-entry">還沒有讀過瓶中信。</div>'}</div>`;
